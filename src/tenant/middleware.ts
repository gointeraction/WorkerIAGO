 ...
 * 
 * Resolution order:
 * 1. X-Tenant-ID header (API clients, admin panel)
 * 2. X-Tenant-Slug header (alternative)
 * 3. Custom domain / subdomain (production)
 * 4. Default tenant (fallback)
 * 
 * Sets: c.set('tenantId', id), c.set('tenant', tenantObj)
 */
  ...
  return async (c: any, next: any) => {
    ...
    if (headerTenantId) {
      ...
      if (headerSlug) {
        ...
        // 3. Subdomain-based resolution (skip for localhost/IP)
